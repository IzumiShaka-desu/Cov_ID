...
class _StatState extends State<Stat> {
  ...
  Widget build(BuildContext context) {
    ...
    var futureBuilderCard = 
    consumerCard(TypeCases.positif);
                    ...
                    Row(children: [
                      ...
                      Column(children: [
                        ...
                        card("sembuh", futureBuilderSembuh, Colors.greenAccent,
                            200),
                        card(
                            "dirawat", futureBuilderAktif, Colors.blueAccent, 200)
                      ])
                    ])
                  ],
                ),
    ...
    return Container(
                        child: Expanded(
                            child: Consumer<ApiProvider>(
                                  ...
                                  builder: (context,
                                      AsyncSnapshot<CoronaGrowth> snapshot) {
  ...
  Container buildContainerPieChart() {
    return Container(
                      ...
                      child: Consumer<ApiProvider>(
                            ...
                            builder:
                                (context, AsyncSnapshot<CoronaModel> snapshot) {
                              Widget pie;
                              switch (snapshot.connectionState) {
                                case ConnectionState.none:
  ...
  Consumer<ApiProvider> consumerCard(TypeCases type) {
    return Consumer<ApiProvider>(builder: (context,apiProvider,_)=>FutureBuilder(
      future: ApiProvider().getDataAsync(),
  ...
  BarChartData createData(CoronaGrowth datas) {
    ...
    List<String> tgl=[];
    data.forEach((element) {tgl.add(element.date);});
    List<BarChartGroupData> dataGroup=[]; 
     int max = data.last.getMax();
    data.forEach((element) {dataGroup.add(createBarRod(BarChartModel(max, element.death, element.recovered, element.active)));});
    return BarChartData(
      groupsSpace: 4,
  ...
  return pieData.map((ChartModel model) {
    return PieChartSectionData(
      titlePositionPercentageOffset: 1,
        ...
        titleStyle: TextStyle(fontSize:12,backgroundColor: Colors.black45,fontWeight: FontWeight.bold),
